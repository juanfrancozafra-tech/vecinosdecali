...
import { AvisoEntregas } from "@/components/AvisoEntregas";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function MisPublicaciones() {
  ...
  const grupos = useMemo(() => {
    ...
    return [
      ...
      { titulo: "Reservados", filas: lista.filter((f) => f.estado === "reservado") },
      {
        ...
        filas: lista.filter(
          (f) =>
            (f.estado === "disponible" && f.solicitudes_abiertas === 0) || f.estado === "oculto",
        ),
      },
  ...
  const planas = useMemo(() => (filas ?? []).filter((f) => pertenece(f, filtro)), [filas, filtro]);
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <div className="max-w-sm text-center">
          ...
          <p className="mt-3 text-body leading-relaxed text-foreground">
            Acá aparecen las publicaciones de quien regala. Si también querés regalar algo, cambiá
            tu cuenta en{" "}
    ...
    <div className="min-h-screen bg-background pb-28">
      <div className="mx-auto w-full max-w-2xl">
        <div className="bg-background px-4 pb-3 pt-6">
          ...
          <AvisoEntregas reservados={reservados} />
